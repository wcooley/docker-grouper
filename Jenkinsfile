    ...
    stages {
                ...
                script {
                  try{
                      ...
                      docker.withRegistry('https://registry.hub.docker.com/',   "dockerhub-tier") {
                         script {
                           sh """
                           docker buildx build . --platform linux/amd64 -t $maintainer/$imagename --build-arg GROUPER_CONTAINER_VERSION=$tag --no-cache --progress plain -f ./Dockerfile
                             """
                            }
        ...
        stage('Test') {
            steps {
                script {
                   try {
                     sh 'echo bin/test.sh 2>&1 | tee debug ; test ${PIPESTATUS[0]} -eq 0'
        ...
        stage('Push') {
            steps {
                script {
                        ...
                        script {
                           sh """
                           docker push $maintainer/$imagename:$tag
                           """
                        }
                        //baseImg.push("$tag")
                      }
